...
var SS = SpreadsheetApp.openById('1dMf0z7_oTz46BsPUsNT8jEBhHTC9asGkdhjjeeczllddfJgot1MA');    //Enter Your Sheet ID Got From Sheet URL Link
var sheet = SS.getSheetByName('BMP280_Data_Sheet');      // Enter your sheet name here, In my case it is Temp_Data_Sheet
var str = "";

function onOpen(){
  var ui = SpreadsheetApp.getUi();
  ui.createMenu('ESP8266_BMP280_Data_Logger')
  .addItem('Clear', 'Clear')
  .addToUi();
}
...
function Clear(){
  ...
  SS.toast('Chart cleared', 'ESP8266_BMP280_Data_Logger', 5);
}
...
//  sheet.getRange('B1').setValue(now);
  sheet.getRange('B1').setValue("Pressure");
   sheet.getRange('C1').setValue("Altitude");
